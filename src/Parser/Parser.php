...
class Parser {
    ...
    /**
     * Parser constructor.
     * @param ServiceLocator $tags
     */
    public function __construct(ServiceLocator $tags) {
        $env = null;
        if (is_null($env)) {
            $env = new BbEnv();
        }
        ...
        while ($entry = array_shift($this->parseArray)) {
            ...
            if (in_array($entry, $stoppers)) {
                ...
                if ($this->bb_mode && $isOpenTag && $exists && $isAllowed) {
                    ...
                    $this->level++;
                    try {
                        $blocks[] = $this->registry->get($tag)->parse($this, $this->env, $arguments);
                    } catch (BbException $ex) {
                        $blocks[] = new BbError($ex->getMessage());
                    }
